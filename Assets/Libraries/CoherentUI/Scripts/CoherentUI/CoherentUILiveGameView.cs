...
/// <summary>
/// Component that needs to be attached to a camera and creates a Coherent Browser Live Game View
/// </summary>
[AddComponentMenu("Coherent Browser/Live Game View")]
public class CoherentUILiveGameView : MonoBehaviour {
	...
	public void Start () {
		...
		if(m_CameraGo != null)
		{
			...
			cameraComponent.targetTexture = m_RenderTexture;
			cameraComponent.aspect = (float)m_Width / (float)m_Height;
		...
		view.Listener.ReadyForBindings += (frameId, path, isMainFrame) => {
			...
			if(m_CameraGo != null)
			{
				...
				camComp.SetTextures(this, m_RenderTexture, m_Texture);
			}
		};
	}
	...
	public void UpdateFromCurrentTexture() {
