...
#endif
	{
		public UnityViewListener(CoherentUIView component, int width, int height)
		{
			...
			this.FailLoad += (frame, path, isMain, errorMsg) =>
			{
				...
				if (m_View == null)
				{
					Debug.LogError("Coherent Browser View is null inside " +
		...
		protected virtual void OnSetMaterial(Material RTMaterial)
		{
			if(m_ViewComponent.gameObject.GetComponent<Renderer>()!=null)
				m_ViewComponent.gameObject.GetComponent<Renderer>().material = RTMaterial;
		}
		protected virtual void OnSetTexture(Texture RTTexture)
		{

			m_ViewComponent.gameObject.GetComponent<Renderer>().material.mainTexture = RTTexture;
		}
		public void OnViewCreatedHandler(View view)
		{
			...
			var cameraComponent = m_ViewComponent.gameObject.GetComponent<Camera>();
			
			var id = m_View.GetId();
			if(cameraComponent)
			{
				ViewRenderer = cameraComponent.gameObject.AddComponent<CoherentUIViewRenderer>();
			...
			else
			{
				...
				var newCameraComponent = renderingCamera.AddComponent<Camera>();
				newCameraComponent.clearFlags = CameraClearFlags.SolidColor;
				newCameraComponent.backgroundColor = new Color(0, 0, 0, 0);
				ViewRenderer = renderingCamera.AddComponent<CoherentUIViewRenderer>();
				...
				OnSetMaterial(RTMaterial);
										
				renderingCamera.transform.parent = m_ViewComponent.gameObject.transform;
			}
			
			ViewRenderer.ViewId = (byte)id;

			var context = m_ViewComponent.Context.UISystem;
			ViewRenderer.ContextId = (byte)(context.GetContextId());
		...
		public void ResizeTexture(int width, int height)
		{
			...
			var camComponent = renderingCamera.GetComponent<Camera>();
			...
			OnSetTexture(RTTexture);

			if(ViewRenderer)
			{
				var c=ViewRenderer.GetComponent<Camera>();
				if(c)
				{
					c.pixelRect=new Rect(c.pixelRect.x,c.pixelRect.y,width,height);
				}
			}
		...
		public void OnJavaScriptMessageHandler(string message, string defaultPrompt, string frameUrl, int messageType)
		{
			...
			var dialogComponent = gameObject.AddComponent<CoherentUIDialog>();
