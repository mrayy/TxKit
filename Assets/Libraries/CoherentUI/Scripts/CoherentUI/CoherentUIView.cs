...
/// <summary>
/// Component containing a Coherent Browser view.
/// </summary>
[AddComponentMenu("Coherent Browser/UI View")]
public class CoherentUIView : MonoBehaviour {
	...
	/// </value>
	[CoherentExposePropertyStandalone(Category = CoherentExposePropertyInfo.FoldoutType.Input,
							PrettyName = "Show virtual keyboard",
							Tooltip="View will show the virtual keyboard for text input widgets",
							IsStatic=true)]
	...
	[CoherentExposePropertyStandalone(Category = CoherentExposePropertyInfo.FoldoutType.Rendering,
							...
							Tooltip="Defines the way the Coherent Browser texture will be filtered on the final Unity texture",
							IsStatic=false)]
	...
	public virtual bool ReceivesInput
	{
		...
		set {
			if (m_Context != null && !m_Context.IsUpdating && this.ClickToFocus)
			{
	...
	[CoherentExposeProperty(
			...
			PrettyName = "View Context",
			Tooltip = "ViewContext to be used by this view",
			IsStatic = true)]
	public CoherentUISystem Context
	{
		get { return m_Context; }
		set {
#if !UNITY_EDITOR
			if(View != null) {
				throw new System.ApplicationException("Context can't be changed"
	...
	public void SetListener(UnityViewListener listener)
	{
		m_Listener = listener;
	...
	public delegate void OnViewCreated_Deleg(CoherentUIView view);
	public OnViewCreated_Deleg OnViewCreatedEvent;
	...
	void Awake () {
		...
		if (m_Context == null)
		{
			m_Context = CoherentUISystem.DefaultContext;
		}
		...
		if (OnViewCreatedEvent != null)
			OnViewCreatedEvent (this);
		else 
			SetListener(new UnityViewListener(this, this.m_Width, this.m_Height));
		
		m_Camera = GetComponent<Camera>();

		m_Context.UISystemDestroying += OnDestroy;
		m_Context.SystemReady += OnSystemReady;
		m_Context.AddView(this);
	}
	...
	private void SendCreateView()
	{
		...
		viewInfo.DisplayVirtualKeyboard = this.m_DisplayVirtualKeyboard;
		viewInfo.UsesSharedMemory = !m_Context.DeviceSupportsSharedTextures
									|| this.m_ForceSoftwareRendering;
		#elif UNITY_IPHONE || UNITY_ANDROID
		...
		{
			m_Context.UISystem.CreateView(viewInfo, Page, m_Listener);
		}
		else
		{
			m_Context.UISystem.CreateView(viewInfo, Page, m_Listener,
		...
		CoherentUIViewRenderer.WakeRenderer(
			(byte)m_Context.UISystem.GetContextId());
		#endif
	}
		...
		if(View != null)
		{
			...
			if(IsOnDemand)
			{
				if(ControlTimeOnDemand)
				{
					...
					{
						arbitraryTime = m_Context.StartTime + Time.time;
					}
	...
	/// <summary>
	/// Destroy this view. Destroys the Coherent Browser view and removes the CoherentUIView
